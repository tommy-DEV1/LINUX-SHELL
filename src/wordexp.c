 ...
 * functions such as wordExpand().
 *
 * returns the malloc'd cmd_token struct, or NULL if insufficient memory.
 */
struct wordStruct *makeWord(char *str)
{
    /* alloc struct memory */
    struct wordStruct *word = malloc(sizeof(struct wordStruct));
    if (!word)
    {
        return NULL;
    }
                    ...
                    int addQuotes)
{
    ...
    tmp = quoteValue(tmp2, addQuotes);
    free(tmp2);
    if (tmp)
    {
        /* substitute the expanded word */
        if ((tmp2 = substituteString(*pstart, tmp, i, i + len)))
        {
...
/*
 * perform word expansion on a single word, pointed to by originalWord.
 *
...
struct wordStruct *wordExpand(char *originalWord)
{
    ...
    if (!*originalWord)
    {
        return makeWord(originalWord);
    }

    char *pstart = malloc(strlen(originalWord) + 1);
    if (!pstart)
    {
        return NULL;
    }
    strcpy(pstart, originalWord);
    ...
    do
    {
        ...
        case '~':
            ...
            if (p == pstart || (in_var_assign && (p[-1] == ':' || (p[-1] == '=' && var_assign_eq == 1))))
            {
                ...
                len = p2 - p;
                substitute_word(&pstart, &p, len, tildeExpand, !in_double_quotes);
                expanded = 1;
            }
            break;
        ...
        case '\'':
            ...
            p += findClosingQuote(p);
            break;

        case '`':
            /* find the closing back quote */
            if ((len = findClosingQuote(p)) == 0)
            {
                /* not found. bail out */
                break;
            }
            ...
            substitute_word(&pstart, &p, len + 1, commandSubstitute, 0);
            expanded = 1;
            break;
        ...
        case '$':
            ...
            case '{':
                ...
                /*
                 *  calling variableExpand() might return an INVALID_VAR result which
                 *  makes the following call fail.
                 */
                if (!substitute_word(&pstart, &p, len + 2, variableExpand, 0))
                {
                    free(pstart);
                    return NULL;
                }
            ...
            case '(':
                 ...
                 */
                func = i ? arithmaticExpand : commandSubstitute;
            ...
            default:
                ...
                substitute_word(&pstart, &p, p2 - p, variableExpand, 0);
                expanded = 1;
                break;
            }
            break;
 ...
 */
char *variableExpand(char *orig_var_name)
{
    /* sanity check */
    if (!orig_var_name)
    {
    ...
    if (!sub) /* we have a substitution without a colon */
    {
        ...
        sub = stringCharacterAny(orig_var_name, "-=?+%#");
    }
    ...
    struct symtabEntryStruct *entry = getSymtabEntry(var_name);
    ...
    else
    {
        ...
        if (sub && *sub)
        {
            ...
            case '%': /* match suffix */
                ...
                /* perform the match */
                if ((len = matchSuffix(sub, p, longest)) == 0)
                {
                    return p;
                }
                /* return the match */
            ...
            case '#': /* match prefix */
                ...
                /* perform the match */
                if ((len = matchPrefix(sub, p, longest)) == 0)
                {
                    return p;
                }
                /* return the match */
    ...
    if (setme)
    {
        ...
        if (!entry)
        {
            entry = addToSymtab(var_name);
        }
        /* and set its value */
        if (entry)
        {
            symtabEntrySetValue(entry, tmp);
        }
    }
 ...
 *    $(command)
 */
char *commandSubstitute(char *orig_cmd)
{
    char b[1024];
    size_t bufsz = 0;
    char *buf = NULL;
...
{
    struct symtabEntryStruct *entry = getSymtabEntry("IFS");
    ...
    struct wordStruct *first_field = NULL;
    struct wordStruct *cur = NULL;
    ...
    do
    {
        ...
        default:
                ...
                is_IFS_char(str[i], IFS_delim) || (i == len))
            {
                ...
                struct wordStruct *fld = malloc(sizeof(struct wordStruct));
...
{
    struct wordStruct *w = words;
    struct wordStruct *pw = NULL;
    ...
    while (w)
    {
        ...
        if (!hasGlobarCharacters(p, strlen(p)))
        {
            pw = w;
            w = w->next;
            continue;
        ...
        glob_t glob;
        char **matches = getFileNameMatches(p, &glob);
        ...
        {
            /* save the matches */
            struct wordStruct *head = NULL, *tail = NULL;
...
void removeQuotes(struct wordStruct *wordlist)
{
    ...
    int in_double_quotes = 0;
    struct wordStruct *word = wordlist;
    char *p;
...
{
    struct wordStruct *w = wordExpand(word);
